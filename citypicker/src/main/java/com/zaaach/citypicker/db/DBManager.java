...
import static com.zaaach.citypicker.db.DBConfig.COLUMN_C_ISHOT;
...
import static com.zaaach.citypicker.db.DBConfig.COLUMN_C_SELECTED;
import static com.zaaach.citypicker.db.DBConfig.COLUMN_C_UPDATE;
...
public class DBManager {
    ...
    public List<HotCity> getHotCities(){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + LATEST_DB_NAME, null);
        Cursor cursor = db.rawQuery("select * from " + TABLE_NAME+" where "+COLUMN_C_ISHOT+" =1 limit 9", null);
        List<HotCity> result = new ArrayList<>();
        HotCity city;
        while (cursor.moveToNext()){
            String name = cursor.getString(cursor.getColumnIndex(COLUMN_C_NAME));
            String code = cursor.getString(cursor.getColumnIndex(COLUMN_C_CODE));
            city = new HotCity(name, false, code);
            result.add(city);
        }
        cursor.close();
        db.close();
    ...
    public List<HotCity> getHistoryCities(){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + LATEST_DB_NAME, null);
        Cursor cursor = db.rawQuery("select * from " + TABLE_NAME+" where "+COLUMN_C_SELECTED+" =1 order by "+COLUMN_C_UPDATE+" desc limit 6", null);
        List<HotCity> result = new ArrayList<>();
        HotCity city;
        while (cursor.moveToNext()){
            String name = cursor.getString(cursor.getColumnIndex(COLUMN_C_NAME));
            String code = cursor.getString(cursor.getColumnIndex(COLUMN_C_CODE));
            city = new HotCity(name, true, code);
            result.add(city);
        }
        cursor.close();
        db.close();
    ...
    public void updateHistoryCity(String sc){
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + LATEST_DB_NAME, null);
        ContentValues values = new ContentValues();
        values.put(COLUMN_C_UPDATE,System.currentTimeMillis());
        values.put(COLUMN_C_SELECTED,1);//key为字段名，value为值
        db.update(TABLE_NAME,values,COLUMN_C_CODE+"=?",new String[]{sc});
        db.close();
    }
    public List<City> searchCity(final String keyword){
        String sql = "select * from " + TABLE_NAME + " where "
                + COLUMN_C_NAME + " like ? " + "or "
                + COLUMN_C_PINYIN + " like ? "+ "or "
                + COLUMN_C_PROVINCE + " like ? ";
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH + LATEST_DB_NAME, null);
        Cursor cursor = db.rawQuery(sql, new String[]{"%"+keyword+"%", keyword+"%", keyword+"%"});
